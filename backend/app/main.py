...
from app.processor import process_documents, aggregate_results, build_metadata_summary
from app.llm import analyze_aggregate
...
async def analyze_topics(files: list[UploadFile] = File(...)) -> FullAnalysisResponse:
    ...
    metadata_summary = build_metadata_summary(aggregated)
    ai_analysis = analyze_aggregate(metadata_summary)

    return FullAnalysisResponse(
        detectedTopics=aggregated["detectedTopics"],
        topicFrequency=aggregated["topicFrequency"],
        filesAppearedIn=aggregated["filesAppearedIn"],
        importanceTier=ImportanceTier(**aggregated["importanceTier"]),
        neverAskedTopics=aggregated["neverAskedTopics"],
        questionPatterns=QuestionPatternResponse(**aggregated["questionPatterns"]),
        aiMetadataSummary=metadata_summary,
